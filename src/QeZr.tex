...
\contenu{
	\texte{
		...
		$\bar{x} = 9.27$ cm, $s^2 = 9.83$ cm$^2$, $s = 3.13$ cm
	...
	\begin{enumerate}
		...
		\reponse{
			...
			\begin{itemize}
				\item Effectif : $n = 25$
				\item Minimum : 4.1 cm
				\item Q1 (25\%) : 6.9 cm (7\textsuperscript{e} valeur)
				\item Médiane (50\%) : 8.9 cm (13\textsuperscript{e} valeur)
				\item Q3 (75\%) : 11.8 cm (19\textsuperscript{e} valeur)
				\item Maximum : 15.1 cm
				\item Écart interquartile : IQR = Q3 - Q1 = 11.8 - 6.9 = 4.9 cm
			\end{itemize}
			...
			$$[6.9 - 1.5 \times 4.9; 11.8 + 1.5 \times 4.9] = [-0.45; 19.15]$$
			...
			\begin{itemize}
				...
				\item La médiane (8.9 cm) est très proche de la moyenne (9.27 cm), 
				ce qui suggère une distribution symétrique
				\item L'écart-type $s = 3.13$ cm représente environ 34\% de la moyenne, 
				ce qui est raisonnable
				\item Pas de valeurs extrêmes selon le critère de Tukey
				\item Distribution unimodale concentrée autour de 9 cm
				\item L'écart-type est nettement inférieur au standard requis (3.13 < 8 cm)
			\end{itemize}
			...
			\begin{center}
				\begin{tikzpicture}
					...
					% Moustaches
					\draw[thick] (4.1,1) -- (6.9,1);
					\draw[thick] (11.8,1) -- (15.1,1);
					
					% Boîte
					\draw[thick,fill=blue!20] (6.9,0.5) rectangle (11.8,1.5);
					...
					\node[below] at (6.9,0.5) {Q1};
					\node[below] at (11.8,0.5) {Q3};
				\end{tikzpicture}
			\end{center}
		}
		...
		\indication{
			...
			\begin{itemize}
				...
				\item Q1 : \texttt{=QUARTILE.INCLURE(plage; 1)} donne 6.9
				\item Médiane : \texttt{=MEDIANE(plage)} ou \texttt{=QUARTILE.INCLURE(plage; 2)} donne 8.9
				\item Q3 : \texttt{=QUARTILE.INCLURE(plage; 3)} donne 11.8
				\item Maximum : \texttt{=MAX(plage)} donne 15.1
				\item IQR : \texttt{=Q3-Q1} donne 4.9
		...
		\reponse{
			...
			Avec $n = 25$, $s^2 = 9.83$ cm$^2$ et $\sigma_0^2 = 64$ cm$^2$ :
			
			$$\chi^2_{\text{obs}} = \frac{(n-1)s^2}{\sigma_0^2} = \frac{24 \times 9.83}{64} 
			= \frac{235.92}{64} \approx 3.69$$
			...
			$\chi^2_{\text{obs}} = 3.69 < 36.415$ : on se trouve dans la région d'acceptation.
			...
			La dispersion observée ($s \approx 3.13$ cm) est \textbf{significativement inférieure} 
		...
		\indication{
			Avec un tableur :
			\begin{itemize}
				\item Statistique : \texttt{=24*9.83/64} donne $3.69$
				\item Valeur critique : \texttt{=LOI.KHIDEUX.INVERSE(0.05; 24)} donne $36.415$
				\item $p$-valeur : \texttt{=1-LOI.KHIDEUX.N(3.69; 24; VRAI)} donne $\approx 0.99999$
			\end{itemize}
		}
		...
		\reponse{
			...
			Avec $n=25$, $s^2 = 9.83$, et les valeurs critiques du $\chi^2_{24}$ :
			\begin{itemize}
				\item $\chi^2_{0.975; 24} = 39.364$
				\item $\chi^2_{0.025; 24} = 12.401$
			\end{itemize}
			
			$$IC_{95\%}(\sigma^2) = \left[\frac{24 \times 9.83}{39.364}; \frac{24 \times 9.83}{12.401}\right] 
			= [5.99; 19.02] \text{ cm}^2$$
			
			\textbf{Intervalle de confiance pour $\sigma$ :}
			
			$$IC_{95\%}(\sigma) = [\sqrt{5.99}; \sqrt{19.02}] = [2.45; 4.36] \text{ cm}$$
			
			\textbf{Interprétation militaire :}
			
			Avec 95\% de confiance, l'écart-type réel de la section se situe entre 2.45 et 4.36 cm.
			
			\begin{itemize}
				\item Le standard de l'armée fixe $\sigma_0 = 8$ cm
				\item La borne supérieure de l'IC est 4.36 cm, \textbf{bien inférieure} à 8 cm
		...
		\indication{
			Avec un tableur :
			\begin{itemize}
				\item Borne inf : \texttt{=RACINE(24*9.83/LOI.KHIDEUX.INVERSE(0.025; 24))}
				\item Borne sup : \texttt{=RACINE(24*9.83/LOI.KHIDEUX.INVERSE(0.975; 24))}
			\end{itemize}
		}
		...
		\item \question{ 			
			...
			\textit{« Votre section montre d'excellents résultats avec un écart-type de 3.1 cm, 
				bien meilleur que le standard de 8 cm. Cependant, je constate que la moyenne 
				des impacts est à 9.3 cm du centre. Ne faudrait-il pas aussi vérifier 
		...
		\reponse{
			...
			$$t = \frac{\bar{x} - \mu_0}{s/\sqrt{n}} = \frac{9.27 - 0}{\sqrt{9.83}/\sqrt{25}} 
			= \frac{9.27}{3.135/5} = \frac{9.27}{0.627} = 14.78$$
			...
			$|t_{\text{obs}}| = 14.78 > 2.064$ : \textbf{on rejette $H_0$}.
			...
			d'environ 9.27 cm par rapport au centre.
		...
		\indication{
			Avec un tableur :
			\begin{itemize}
				\item Écart-type : \texttt{=RACINE(9.83)} donne $3.135$ cm
				\item Statistique $t$ : \texttt{=9.27/(3.135/RACINE(25))} donne $14.78$
				\item Valeur critique : \texttt{=LOI.STUDENT.INVERSE.BILATERALE(0.05; 24)} donne $2.064$
				\item $p$-valeur : \texttt{=LOI.STUDENT.BILATERALE(14.78; 24)} donne $< 10^{-13}$
			\end{itemize}
		}
		...
		\reponse{
			...
			Statistique de test :
			$$F = \frac{s_y^2}{s^2} = \frac{15.3}{9.83} = 1.556$$
			...
			les valeurs critiques correspondantes.
			
			En pratique, pour un test bilatéral avec $F \geq 1$, on compare avec $F_{1-\alpha/2}$ :
			$$F_{0.975; 19, 24} \approx 2.35$$
			
			\textbf{Décision :}
			
			$F_{\text{obs}} = 1.556 < 2.35$ : \textbf{on ne rejette pas $H_0$}.
		...
		\indication{
			Avec un tableur :
			\begin{itemize}
				\item Statistique $F$ : \texttt{=15.3/9.83} donne $1.556$
				\item Valeur critique : \texttt{=LOI.F.INVERSE.DROITE(0.025; 19; 24)} donne $\approx 2.35$
				\item $p$-valeur : \texttt{=2*MIN(LOI.F.N(1.556; 19; 24; VRAI); 1-LOI.F.N(1.556; 19; 24; VRAI))}
			\end{itemize}
		}
	\end{enumerate}
}