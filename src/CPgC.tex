...
\end{tikzpicture}
	...
	\texte{Le cosinus hyperbolique et le sinus hyperbolique sont la partie paire et impaire de l'exponentielle :
...
\begin{enumerate}
	\item \question{ Démontrer les propriété suivante : }
		...
		\begin{enumerate}
			\item \question{ $\ch^2(x) - \sh^2(x) = 1$, pour tout $x \in \R$. }
			\reponse{$\ch^2 x- \sh^2 x = \frac14 \big[ (e^x+e^{-x})^2 - (e^x-e^{-x})^2 \big] = 
				\frac14 \big[ (e^{2x}+2+e^{-2x}) - (e^{2x}-2+e^{-2x})  \big] = 1$.}
			\item \question{ $\ch '(x) = \sh(x)$ et $\sh'(x) = \ch(x)$. }
			\reponse{$\frac{d}{dx}(\ch x) = \frac{d}{dx} \frac{e^x+e^{-x}}{2} = \frac{e^x-e^{-x}}{2}  = \sh x$.}
		\end{enumerate}
   \item \question{Les fonctions trigonométriques $\cos$ et $\sin$ sont dites << circulaires >>. On rappelle les formules d'Euler :
   ...
   \item \question{\'Etant donné un arc paramétré $\begin{array}[t]{cccc}
   ...
   $$L = \int_a^b \sqrt{x'(t)^2 + y'(t)^2} \dt$$
   	...
   	\begin{enumerate}
   		...
   		\reponse{
   			...
   			\end{array}$$}
   	\end{enumerate}
   \item \question{On peut démontrer les propriétés suivantes des fonctions hyperboliques :}

   	\begin{enumerate}
   		\item \question{La fonction $x \mapsto \ch x$ est une bijection
   		...
   		\item \question{La fonction $x \mapsto \sh x$ est une bijection
   		de $\Rr$ dans $\Rr$. Sa bijection réciproque est 
   		notée $\Argsh x$. Donc :
   		$$\sh\left(\Argsh(x)\right) = x  \qquad\qquad  \Argsh\left(\sh(x)\right) = x
   		\quad   \forall x \in \Rr$$}
   		\item \question{Les fonctions $x \mapsto \Argch(x)$ et $x \mapsto \Argsh(x)$
   		sont dérivables et 
   		...
   		\reponse{Comme la fonction $x \mapsto \ch'x$  ne s'annule pas sur $]0,+\infty[$
   			...
   			= \frac{1}{\sqrt{x^2-1}}.$$  }
   		\item \question{Pour tout $x \geq 1$ : $\Argch(x)  =  \ln\left( x+\sqrt{x^2-1} \right)$.}
   		...
   		\item \question{Pour tout $x \in \R$ : $\Argsh(x)  =  \ln\left( x+\sqrt{x^2+1} \right)$.}
   		\reponse{ Notons $f(x)=\ln\big(x+ \sqrt{x^2+1}\big)$. 
