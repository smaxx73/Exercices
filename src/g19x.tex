...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item \question{  Montrer que $4 |x_i| \leq 2 \|x\|_\infty$ pour $i \in \{1, \ldots, n\}$. }
		
		\item \question{ En déduire que $x = 0$. }
		
	\end{enumerate}
	
	\item \question{ Montrer que le syst\`eme ci-dessus peut s'\'ecrire $Ax=b$ avec une matrice $A$ que l'on donnera pour $n=4$. Donner un code Python qui permet de générer la matrice $A$ pour $n$ quelconque. }
