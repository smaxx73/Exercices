...
\contenu{
	...
	\begin{enumerate}
		...
		\indication{Commencer par diviser l'inégalité par $0{,}30$, puis centrer-réduire les deux bornes obtenues avec $Z=(W-\mu)/\sigma$.}
		\reponse{
		...
		\indication{Reprendre exactement la méthode de la première question, en remplaçant le coefficient $0{,}30$ par $0{,}45$.}
		\reponse{
		...
		\indication{Transformer d'abord l'intervalle de charges en un intervalle de masses $W$, puis utiliser une différence de valeurs de la fonction $\Phi$.}
		\reponse{
		...
		\indication{Une transformation affine d'une loi normale reste normale. Écrire donc la moyenne et l'écart-type de $L_f$, puis utiliser le quantile $z_{0{,}95}\simeq1{,}645$.}
		\reponse{
			$L_f \sim \mathcal{N}(0,30\mu, (0,30\sigma)^2) = \mathcal{N}(56,57, 9,41^2)$ (lb). Donc $q_{0,95}(L_f) = 56,57 + 1,645 \times 9,41 \approx 72,0$ lb. \\
			\textbf{Lecture :} 95\% des soldats auraient un fighting load (30\%) $\leq 72$ lb, bien à l'intérieur de la cible 60-80 lb.
		}
	\end{enumerate}
}
