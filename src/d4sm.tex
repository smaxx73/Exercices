...
\contenu{
    On dit qu'une variable aléatoire $ X $ suit une loi de Pareto de paramètre $(\alpha, a, x_0)$ si elle admet pour densité la fonction $ f $ définie par : 
    $$ f(x) = \mathbf{1}_{]x_0 + a ;+\infty[}(x) \frac{\alpha}{a} \left( \frac{a}{x} \right)^{\alpha+1} $$
    ...
    \begin{enumerate}
        ...
        \item \question{Étudier l'existence de la valeur éventuelle de $ \E(X) $.}
        \reponse{
    ...
    Soient $ X $ une variable aléatoire suivant une loi de Pareto de paramètre $(\alpha,1 ,0)$, et des réels $ r > 0 $ et $ s $. Soit alors la variable aléatoire $ Y = rX + s $.
    
    \begin{enumerate}
        \setcounter{enumi}{4} % Commencer à partir de la question 5
        \item \question{Étudier la loi suivie par la variable aléatoire $ Y = rX + s $.}
        \reponse{
            ...
            \end{align*}
            Par dérivation, on trouve que la densité de $Y$ est $ f_Y(x) = \mathbf{1}_{]s+1;+\infty[}(x) \frac{\alpha}{r} \left( \frac{r}{x-s} \right)^{\alpha+1} $. On trouve une loi de Pareto de paramètre $(\alpha, r, s+1)$.
