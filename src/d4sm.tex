...
\contenu{
    ...
    \begin{enumerate}
        ...
        \indication{Dans l'intégrale définissant $\E(X)$, simplifier $x f(x)$ puis appliquer le critère de convergence des intégrales de Riemann à l'infini.}
        \reponse{
    ...
    \texte{Soit $ X $ une variable aléatoire suivant une loi de Pareto de paramètre $(\alpha,1 ,0)$ et soient deux réels $r,s$ tels que  $r > 0$. On pose alors la variable aléatoire $ Y = rX + s $.}
        ...
        \indication{Comme $r>0$, la transformation $x\mapsto rx+s$ est croissante. Écrire $F_Y(t)=\prob\left(X\leq\frac{t-s}{r}\right)$ et distinguer les valeurs de $t$ selon le support.}
        \reponse{
