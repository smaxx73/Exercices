...
\end{center} }
\begin{enumerate}
	\item \question{ Quelle est la seule valeur possible du paramètre $a$ ? }
	...
	\reponse{L'espérance de $X$ est la suivante:
		...
		\E(X)&= (-2)\prob(X=-2)+(-1)\prob(X=-1)+1\prob(X=1)+2\prob(X=2) \\
			&= (-2)\times \frac{5}{18}+(-1)\times \frac{2}{18}+\frac{4}{18}+2\times \frac{7}{18} \\
		&=\frac{1}{3}.
		\end{align*}
	...
	\item \question{ Déterminer $y$ de sorte que $\E(XY)=0$. }
	...
	\indication{Déterminer dans un premier temps les valeurs prises par le couple $(U,V)$. }
