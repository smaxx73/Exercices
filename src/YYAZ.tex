...
\contenu{
    ...
    \indication{Noter $D$ l'événement « défectueux » et $T$ l'événement « déclaré mauvais ». Calculer d'abord $\prob(T)$ par la formule des probabilités totales, puis appliquer Bayes pour $\prob(D\mid T)$.}
    ...
    \reponse{ On note $D$ l'événement \og le composant est défectueux \fg{} et $T$ l'événement \og le testeur déclare le composant en mauvais état \fg{}. On cherche à calculer $\prob(D|T)$. 
	    ...
	    D'après l'énoncé, on a : $\prob(D) = 0{,}02$, $\prob(T|\overline{D}) = 0{,}05$ et $\prob(T|D) = 0{,}94$. On a donc :
