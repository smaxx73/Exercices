...
\difficulte{3}
\contenu{
\texte{On effectue un contrôle sur des pièces d'un euro dont une proportion $p = 0{,}05$ est fausse. On considère un lot de 1000 pièces d'un euro. Soit $X$ la variable aléatoire égale au nombre de pièces fausses parmi les 1000.}
...
\begin{enumerate}
    ...
    \indication{L'événement $\{X>100\}$ est contenu dans $\{|X-50|>50\}$. Appliquer Bienaymé--Tchebychev à ce dernier événement.}
    \reponse{
    ...
    \indication{Voir $X$ comme la somme de $1000$ indicatrices de Bernoulli indépendantes et vérifier que les effectifs attendus $np$ et $n(1-p)$ sont suffisamment grands.}
    \reponse{
    ...
    \indication{Utiliser la normale approchante avec une correction de continuité : remplacer l'intervalle entier $[48,52]$ par $[47{,}5,52{,}5]$, puis centrer-réduire.}
    \reponse{
        On calcule : 
        \begin{align*}
