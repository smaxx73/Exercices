...
\begin{enumerate}
	...
	\item  \question{On définit la fonction $\varphi_1 \colon x \mapsto x-\frac{x(1+e^x)-e^x}{1+xe^x}$ et pour un $x_0$ quelconque dans $]0,1[$, on définit une suite $(x_n)$ par la relation de récurrence suivante : 
...
%			$$}
	...
	\texte{et la suite
	$$
	\texte{x_{n+1}=\varphi_2(x_n),\quad x_0\in [0,1].
	$$
	\begin{enumerate}
