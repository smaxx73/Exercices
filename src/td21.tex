...
\texte{
Soit un échantillon i.i.d. $(X_i)_{1 \leq i \leq n}$ tel que $E(X_1) = m$ et $Var(X_1) = \sigma^2$. On suppose $\sigma^2$ connue, $m \in \mathbb{R}$ étant ici le paramètre inconnu. On propose deux estimateurs de $m$ : $\bar{X}_n$ et $Z_n = \frac{1}{2}(X_n + X_{n-1})$.
}

\begin{enumerate}
    \item \question{Montrer que $\bar{X}_n$ et $Z_n$ sont sans biais.}
    \indication{}
    \reponse{On a :
\begin{equation}
...
Ainsi, les deux estimateurs sont sans biais pour l'estimation de $m$.}
    \item \question{Qui de $\bar{X}_n$ ou $Z_n$ choisiriez-vous pour approcher $m$ ?}
    \indication{}
...
\begin{equation}
    EQM(X_n) = 0 + \text{Var}(X_n) = \frac{\sigma^2}{n}
\end{equation}
\begin{equation}
    EQM(Z_n) = 0 + \frac{1}{4} \text{Var}(X_n + X_{n-1}) = \frac{\sigma^2}{2}.
\end{equation}
Pour tout $n \geq 3$, on a $EQM(X_n) < EQM(Z_n)$, donc $X_n$ est préférable à $Z_n$ au sens du risque quadratique.}
