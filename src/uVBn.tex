...
\texte{Etant donnés trois nombres réels dans l'intervalle $[0;1]$, on définit deux stratégies :
	...
	Deux personnes jouent au jeu comportant les étapes suivantes :
...
\begin{enumerate}
	\item \question{Quelle la loi de probabilité suivie par chaque variable aléatoire $X_i$, $i \in \{1,2,3\}$ ?}
	...
	\item \question{Déterminer la fonction de répartition de $Y_A$. En déduire que $Y_A$ est une variable aléatoire absolument continue dont on déterminera la densité.}
	...
	\item \question{Y a-t-il une meilleure stratégie ?}
