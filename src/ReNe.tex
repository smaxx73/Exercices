\uuid{ReNe}
\titre{Calcul du gradient de la fonction d'erreur}
...
\item On pose la fonction d'erreur $E(w)=(y-\hat{y})^2.$ En appliquant l'algorithme de backpropagation, déterminer les dérivées partielles $\frac{\partial E(w)}{\partial w_j}$ puis $\frac{\hat{y}}{\partial w_j}$. En déduire les expressions des mises à jour des paramètres $\Delta w_j$ pour $j=1,\cdots{},7$. 
