...
\begin{enumerate}
	\item \question{ Vérifier que $H_n$ est une application contractante et en déduire que le schéma est bien défini. }
	\reponse{$|H_n'(x) = h \left| \frac{\partial f}{\partial y}(t_{n+1},x)  \right| \leq hM \leq \frac{1}{2} < 1$.}
	...
	\end{cases}$$
	On admet que cette méthode est stable. Expliquer ce choix puis montrer qu'elle est convergente. }
	...
	\begin{enumerate}
		...
		\reponse{\begin{align*}
				...
				&\leq 4M(1+2Mh)|y-z|
		\end{align*} }
		\item \question{ En déduire que la méthode est stable, consistante et convergente. }
		\reponse{On étudie la consistance du nouveau schéma $(S')$ : il s'écrit $y_{n+1}=y_n+hF(y_n,h)$ avec $F(y,h)=f(y+hg_R(y,h))$. Or $f$ est $M$-lipschitzienne donc 
			\begin{align*} F(y,h)-F(z,h) &\leq M(|y-z|+h|g_R(y,h)-g_R(z,h)|)\\
				&\leq M(1+2Mh(1+2Mh))|y-z|\\
				&\leq 5M|y-z|
			\end{align*}
			Donc $F$ est lipschitzienne uniformément en $h$ : cela prouve la stabilité du schéma. La consistance ayant déjà établie dans le cas général, cela prouve que la méthode est convergente.}
	\end{enumerate}
\end{enumerate}