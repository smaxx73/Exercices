...
On en déduit que $\prob(X=1)=\frac{1}{21}$ et donc que pour tout $k \in \{1,2,3,4,5,6\}$, on a $\prob(X=k)=\frac{k}{21}$.
 ...
 \reponse{
    Les valeurs prises par $Y$ sont les inverses des valeurs prises par $X$. Ainsi, on a :
    \begin{align*}
        \prob(Y=\frac{1}{1}) &= \prob(X=1) \\
        \prob(Y=\frac{1}{2}) &= \prob(X=2) \\
        \prob(Y=\frac{1}{3}) &= \prob(X=3) \\
        \prob(Y=\frac{1}{4}) &= \prob(X=4) \\
        \prob(Y=\frac{1}{5}) &= \prob(X=5) \\
        \prob(Y=\frac{1}{6}) &= \prob(X=6).
    \end{align*}

    Pour calculer l'espérance, on procède de la même manière que dans la question précédente :

    \begin{align*}
        \E(Y) &= \sum_{k=1}^6 \frac{1}{k} \prob(Y=\frac{1}{k}) \\
