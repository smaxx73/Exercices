...
\begin{enumerate}
	...
	\reponse{Il suffit de dériver une seconde fois par rapport à $x$ l'expression de $\frac{\partial T}{\partial x}(x,t)$ : on trouve
		...
		&= (-\lambda)^2 T_1 e^{-\lambda x} \left( \sin(\omega t-\lambda x)+ \cos(\omega t-\lambda x)  \right)\\
		&\qquad + (-\lambda)^2 T_1 e^{-\lambda x} \left(\cos(\omega t-\lambda x)- \sin(\omega t-\lambda x)\right) \\
