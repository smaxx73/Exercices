...
\reponse{ On a $g(x_n)=x_{n+1}$ et $g(x_{n-1}) = x_n$, donc $|x_{n+1}-x_n| = |g(x_n)-g(x_{n-1})|$. L'in\'egalit\'e demand\'ee est l'in\'egalit\'e des accroissements finis.
	\item La question pr\'ec\'edente permet de montrer par r\'ecurrence sur $p \geq 0$ que $|x_{p+1}-x_p| < M^p |x_1-x_0|$. De plus, l'in\'egalit\'e triangulaire permet d'\'ecrire que pour tout $k < l$, on a 
	\begin{equation*}
	...
	\end{equation*}
	Comme $0<M<1$, on sait que la suite $\frac{M^k}{1-M} |x_1-x_0|$ a pour limite $0$ lorsque $k \rightarrow \infty$. Ainsi, pour tout $\ve>0$, il existe un rang $K$ tel que pour tous $K \leq k \leq l$, on a $|x_l-x_k|<\ve$. La suite $(x_n)_{n \geq 0}$ est donc une suite de Cauchy, et comme $\RR$ est complet, on sait qu'elle est convergente. }
