...
\begin{enumerate}
	\item \question{ La fonction $f$ est-elle continue en $(0,0)$~? }
		\indication{On peut passer en coordonnées polaires en posant $x=r\cos(\theta)$ et $y=r\sin(\theta)$ et montrer la continuité en majorant $|f(x,y)-f(0,0)|$ par une fonction de $r$ qui tend vers $0$ quand $r \to 0$. }
	...
	\reponse{Pour $(x,y) \neq (0,0)$, on  applique les formules classiques de dérivation. 
		\begin{align*}
	...
	\indication{Il faut revenir à la définition avec le calcul des taux d'accroissement car on ne peut pas appliquer les formules de dérivation au voisinage de $(0,0)$. }
	\reponse{
		On revient à la définition en étudiant les taux d'accroissement.
	...
	\indication{Il s'agit de vérifier que les dérivées partielles de $f$ sont continues en $(0,0)$.}
	\indication{
	Pour établir si les dérivées partielles de $f$ sont continues en $(0,0)$, on doit étudier :
	\begin{itemize}
		\item si $\dpa{f}{x}(x,y)$ (resp. $\dpa{f}{y}(x,y)$) admet une limite quand $(x,y) \to (0,0)$ puis
		\item si cette limite est égale à $\dpa{f}{x}(0,0)$ (resp. à $\dpa{f}{y}(0,0)$)
	\end{itemize}
	}
	\reponse{Pour établir si $f$ est $C^1$ en $(0,0)$, on doit étudier
		\begin{itemize}
			\item si $\dpa{f}{x}(x,y)$ (resp. $\dpa{f}{y}(x,y)$) admet une limite quand $(x,y) \to (0,0)$ puis
	...
	\reponse{Comme $f$ admet des dérivées partielles en $(0,0)$, il est possible que $f$ soit différentiable en $(0,0)$ et si elle l'est, sa différentielle $\dd f(0,0)$ est nécessairement égale à $-3 \dd x + 5 \dd y$. Puisque $f$ n'est pas de classe $C^1$, on ne peut pas conclure directement à la différentiabilité et il faut étudier la limite de
		\[ \frac{f(h,k) - f(0,0) - [(-3)h + 5k]}{\sqrt{h^2+k^2}} \]
		quand $(h,k) \to (0,0)$. Or 
		\begin{align*}
