\titre{Domaine de convergence et somme d'une série entière}
\theme{Série}
\auteur{}
\organisation{AMSCC}	
...
\begin{array}{cl}
	...
	0& \textrm{si $x =0$}\\
	-1&\textrm{si $x \in [-\pi;0[$}\end{array}\right.$$ }

\begin{enumerate}
	\item \question{ Représenter graphiquement l'allure de la fonction $f$ et vérifier que $f$ est une fonction impaire. }
	\item\question{  Déterminer les coefficients de Fourier trigonométriques de la fonction $f$ et en déduire sa série de Fourier. }
	\reponse{C'est une fonction impaire donc $a_n = 0$ pour tout $n$. Il reste à calculer :
		$$b_n=\frac{1}{\pi}\int_{-\pi}^0(-1)\sin(nx)dx+\frac{1}{\pi}\int_0^\pi\sin(nx)dx=\frac{2}{n\pi}(1-\cos(n\pi)).$$ soit $$b_n=\left\{
		\begin{array}{cl}
			0&\textrm{pour $n$ pair}\\
