...
\begin{enumerate}
	...
	\indication{Centrer-réduire chaque information donnée, puis lire les deux quantiles correspondants dans la table de $\mathcal N(0,1)$. On obtient pour chaque joueur un système de deux équations en $\mu$ et $\sigma$.}
	\reponse{ 
		\textbf{Joueur $A$:} 
	...
	\indication{Pour des variables normales indépendantes, une combinaison linéaire est normale : les moyennes se soustraient et les variances s'additionnent.}
	...
	\indication{L'événement s'écrit $\{X>Y\}=\{Z>0\}$. Centrer-réduire ensuite $Z$ avant la lecture dans la table normale.}
