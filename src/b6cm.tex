...
\begin{enumerate}
    ...
    \reponse{
        ...
        \begin{align*}
            \prob(X \geq t) &= \int_t^{+\infty} f(x) \dx \\
            &= \int_t^{+\infty} 3 e^{-3x} \dx \\
    ...
    \indication{Pour $t\geq0$, l'événement $\{\min(X,Y)\geq t\}$ est l'intersection $\{X\geq t\}\cap\{Y\geq t\}$. Utiliser l'indépendance puis les fonctions de survie.}
    \reponse{
        Soit $t \in \R$. Si $t \geq 0$, on a : 
        \begin{align*}
