...
\contenu{
	\texte{
		...
		\textit{Pour gagner du temps, vous pouvez utiliser : $\sigma(0) = 0.5$, $\sigma(1) \approx 0.731$, $\sigma(-1) \approx 0.269$, $\sigma(2) \approx 0.88$, $\sigma(-2) \approx 0.12$, $\tanh(2) \approx 0.96$, $\tanh(0) = 0$.}
	}
	\begin{enumerate}
		\item
		...
		\reponse{
			...
			\]
			\[
			h_1 = 0.731 \times \tanh(0.845) \approx 0.731 \times 0.688 \approx 0.503
			\]
		}
		...
		\reponse{
			...
			\]
			\[
			c_2 = 0.5 \times 0.845 + 0.88 \times 0.96 \approx 0.423 + 0.845 = 1.268
			\]
			\[
			h_2 = 0.731 \times \tanh(1.268) \approx 0.731 \times 0.853 \approx 0.624
			\]
		...
		\item
		\question{Dans un RNN simple, l'état caché $h_t$ était directement écrasé à chaque pas. Expliquer en quoi le mécanisme de cell state $c_t$ de la LSTM permet de mieux préserver l'information sur le long terme.}
		\reponse{
			Dans le RNN simple, $h_t = \tanh(\cdots)$ est une transformation non linéaire saturante de $h_{t-1}$ : l'état précédent est systématiquement « compressé » dans $[-1, 1]$, ce qui fait que le gradient peut facilement tendre vers $0$ au fil des pas de temps (phénomène du \textit{gradient vanishing}).
