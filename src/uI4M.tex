...
\contenu{
	...
	\begin{enumerate}
		...
		%-------------------------------------------------------
		\item \question{Représenter graphiquement $f$ sur $[-\pi,\pi]$, déterminer sa parité,
			et en déduire lequel de $\cos(x)$ ou $\sin(x)$ est adapté pour l'approcher.
			Que conjecture-t-on sur le coefficient $a$ ?}
		\reponse{La fonction $f : x \mapsto x$ est la droite passant par l'origine, de pente $1$,
			tracée sur $[-\pi,\pi]$.
			
			Pour tout $x$, $f(-x) = -x = -f(x)$, donc $f$ est impaire.
			
			$\cos(x)$ est paire, $\sin(x)$ est impaire comme $f$ : c'est donc $\sin(x)$ qui est
			adapté. Les termes pairs n'ont pas leur place dans l'approximation, on conjecture $a = 0$.}
		
		%-------------------------------------------------------
		\item \texte{On choisit donc une approximation de la forme $S_1(x)=b\sin(x)$.
			On souhaite choisir $b$ de sorte que l'erreur quadratique intégrale
			$$
			E(b) = \int_{-\pi}^{\pi}\bigl(f(x)-b\sin(x)\bigr)^2\,dx
			$$
			soit minimale.}
		\question{Montrer que minimiser $E(b)$ revient à imposer la condition d'orthogonalité suivante :
			$$
		...
		%-------------------------------------------------------
		\item 
		\question{Calculer $\displaystyle\int_{-\pi}^{\pi} \sin^2(x)\,dx$, puis
			$\displaystyle\int_{-\pi}^{\pi} x\sin(x)\,dx$ par intégration par parties.
			En déduire la valeur de $b$ telle que $E(b)$ est minimale puis l'expression de $S_1$.}
		\indication{Pour l'intégration par parties, poser $u=x$ et $v'=\sin(x)$.}
		\reponse{En utilisant $\sin^2(x)=\dfrac{1-\cos(2x)}{2}$ :
			$$
			\int_{-\pi}^{\pi} \sin^2(x)\,dx
			= \frac{1}{2}\Bigl[x - \frac{\sin(2x)}{2}\Bigr]_{-\pi}^{\pi}
			= \pi.
			$$
			On pose $u=x$, $v'=\sin(x)$, donc $u'=1$, $v=-\cos(x)$ :
			\begin{align*}
				\int_{-\pi}^{\pi} x\sin(x)\,dx
				&= \Bigl[-x\cos(x)\Bigr]_{-\pi}^{\pi} + \int_{-\pi}^{\pi}\cos(x)\,dx \\
				&= \bigl(-\pi(-1)-(-\pi)(-1)\bigr) + \Bigl[\sin(x)\Bigr]_{-\pi}^{\pi} \\
				&= \bigl(\pi + \pi\bigr) + 0 = 2\pi.
			\end{align*}
			La condition d'orthogonalité donne $b \cdot \pi = 2\pi$, donc $b = 2$.
			La première approximation est $S_1(x) = 2\sin(x)$.}
		
		%-------------------------------------------------------
		\item \texte{On cherche maintenant une approximation plus précise de $f$, sous la forme
			$$
			S_2(x)=b_1\sin(x)+b_2\sin(2x).
			$$
			On impose que l'erreur $f(x)-S_2(x)$ soit orthogonale à $\sin(x)$ et à $\sin(2x)$.}
		\question{Écrire les deux conditions intégrales correspondantes.}
		\reponse{
			$$
		...
		%-------------------------------------------------------
		\item \question{Montrer que $\displaystyle\int_{-\pi}^{\pi} \sin(x)\sin(2x)\,dx=0$,
			puis en déduire que les deux coefficients $b_1$ et $b_2$ peuvent être déterminés
			séparément par
			$$
		...
		\reponse{Par la formule produit-en-somme :
			$$
			\sin(x)\sin(2x) = \frac{1}{2}\bigl[\cos(x)-\cos(3x)\bigr],
			$$
			donc
			$$
			...
			Grâce à cette orthogonalité, les deux conditions intégrales se découplent. La première donne
			$$
		...
		%-------------------------------------------------------
		\item \question{Calculer $b_2$ par intégration par parties. On admet que, pour tout entier
			$n\geqslant 1$,
			$$
			\int_{-\pi}^{\pi} x\sin(nx)\,dx = \frac{2\pi(-1)^{n+1}}{n}.
			$$
			En déduire une expression générale de $b_n$.}
		\reponse{On intègre par parties $\int_{-\pi}^{\pi}x\sin(2x)\,dx$, avec $u=x$,
			...
			En appliquant la formule admise :
			$$
			b_n = \frac{1}{\pi}\cdot\frac{2\pi(-1)^{n+1}}{n} = \frac{2(-1)^{n+1}}{n}.
			$$
			On vérifie : $b_1=2$, $b_2=-1$, $b_3=\frac{2}{3}$, cohérent avec les calculs précédents.}
		...
		\item \texte{
			...
			$$
			\emph{Note sur la convergence.} Cette égalité repose sur le théorème de Dirichlet,
			admis ici. On ne peut pas l'établir à l'aide des seuls critères des séries numériques,
			car les termes $\frac{2(-1)^{n+1}}{n}\sin(nx)$ oscillent simultanément en $n$ et en $x$.}
		\question{Comparer cette écriture avec les développements en série entière déjà rencontrés :
		...
		\reponse{En $x=\dfrac{\pi}{2}$ :
			...
			1-\frac{1}{3}+\frac{1}{5}-\frac{1}{7}+\cdots = \frac{\pi}{4}.
			$$
			Il s'agit de la formule de Leibniz-Madhava, retrouvée comme cas particulier
