\uuid{uI4M}
\titre{Introduction aux séries de Fourier — Retrouver une fonction périodique à partir d'une somme trigonométrique}
\niveau{L2}
\module{Analyse}
...
\contenu{
	...
	\texte{
		On connaît déjà les séries numériques et les séries entières, qui permettent d'écrire certaines
		fonctions sous la forme d'une somme infinie. On cherche ici à comprendre une autre idée : peut-on
		reconstruire une fonction périodique à partir des fonctions
		\[
	...
	\begin{enumerate}
		...
		%-------------------------------------------------------
		\item \texte{\textbf{Une première approximation.}
		...
		\begin{enumerate}
			...
			\item \question{Représenter qualitativement la fonction $f$ sur $[-\pi,\pi]$.}
			...
			\item \question{On choisit donc une approximation de la forme $S_1(x)=b\sin(x)$.
				On souhaite choisir $b$ de sorte que l'erreur $f(x)-b\sin(x)$ soit, en un certain sens,
				aussi petite que possible.
				
				\emph{Note.} On peut montrer que la condition ci-dessous est équivalente à minimiser
				l'erreur quadratique intégrale $\displaystyle\int_{-\pi}^{\pi}\bigl(f(x)-b\sin(x)\bigr)^2\,dx$.
				C'est en ce sens que $b$ sera « optimal ».}
			\reponse{Cette remarque prépare la condition d'orthogonalité introduite à la partie suivante.
				Elle évite que le choix de la condition intégrale paraisse arbitraire.}
		...
		\begin{enumerate}
			...
			\item \question{Montrer que cette condition équivaut à
				\[
				\int_{-\pi}^{\pi} f(x)\sin(x)\,dx = b\int_{-\pi}^{\pi} \sin^2(x)\,dx.
				\]}
			\reponse{Par linéarité de l'intégrale :
				\[
				\int_{-\pi}^{\pi} \bigl(f(x)-b\sin(x)\bigr)\sin(x)\,dx
				= \int_{-\pi}^{\pi} f(x)\sin(x)\,dx - b\int_{-\pi}^{\pi} \sin^2(x)\,dx = 0,
				\]
				ce qui donne bien l'égalité annoncée.}
		...
		%-------------------------------------------------------
		\item \texte{\textbf{Bilan.}
			
			Pour approcher une fonction périodique, on peut chercher à la décomposer comme une somme
			de fonctions trigonométriques :
			\[
			f(x) \sim \frac{a_0}{2} + \sum_{n\geqslant 1}\bigl(a_n\cos(nx)+b_n\sin(nx)\bigr).
			\]
			Le terme $\dfrac{a_0}{2}$ correspond à la « valeur moyenne » de $f$ sur $[-\pi,\pi]$,
			obtenue en appliquant la même condition d'orthogonalité à la fonction constante $1$
			(le facteur $\tfrac{1}{2}$ est une convention qui uniformise la formule).
			Les nombres $a_n$ et $b_n$ sont appelés \textbf{coefficients de Fourier} de $f$.
			Ils mesurent la « quantité » de chaque oscillation $\cos(nx)$ ou $\sin(nx)$ présente dans $f$.
			
			Dans le cas général, on montrera que
			\[
