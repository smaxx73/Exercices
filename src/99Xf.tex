...
\contenu{
	...
	\begin{enumerate}
		\item \question{ \'Ecrire $Y=X+U$ où $U$ est une variable aléatoire indépendante de $X$ dont la loi reste à préciser. }
		      \reponse{ Soit $U$ la variable aléatoire de Bernoulli égale à $1$ si on a choisi le jeu 2 et que le $2n$-ième lancer donne ``pile''. Comme les lancers sont indépendants, la variable aléatoire $U$ est indépendante de $X$ et on a bien $Y=X+U$, avec $U \sim \mathcal{B}(p)$. }
