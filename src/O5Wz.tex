...
\begin{enumerate}
    ...
    \indication{L'événement demandé est l'intersection des trois $A_i$. Les variables étant indépendantes, multiplier les trois probabilités de survie.}
    \reponse{
    ...
    \item \question{Soit $Y$ la variable aléatoire égale à la plus grande des 3 durées de fonctionnement de la machine sans interruption.
    \begin{enumerate}
        ...
        \indication{Écrire $Y=\max(X_1,X_2,X_3)$ : l'événement $\{Y\leq t\}$ impose simultanément $X_i\leq t$ pour les trois indices.}
        \reponse{
        ...
        \indication{Dériver la fonction de répartition sur $t>0$ et ne pas oublier que la densité est nulle hors de $[0,+\infty[$.}
        \reponse{
        ...
        \indication{Développer le carré dans $f_Y(t)$ pour ramener $\E(Y)$ à une combinaison d'intégrales $\int_0^{+\infty}t e^{at}\,\mathrm dt$ de la question précédente.}
        \reponse{
