...
\begin{enumerate}
	...
	\item \question{ En déduire que $M_n \xrightarrow[]{\text{p.s.}} 1$ et $M_n \xrightarrow[]{\text{en loi}} 1$. }
	      \reponse{ On remarque que pour tout $\omega \in \Omega$, la suite $(M_n(\omega))$ est une suite réelle croissante. Cette suite est également majorée par $1$ puisque chaque variable $U_i$ est majorée par $1$. La suite $(M_n(\omega))$ est donc une suite convergente, notons $L(\omega)$ sa suite. Il existe donc une variable aléatoire $L$ telle que la suite $(M_n)$ converge vers $L$ presque sûrement. Or on sait que la convergence presque sûre implique la convergence en probabilité. Et d'après la question précédente, la suite $(M_n)$ converge en probabilité vers la variable aléatoire constante $1$. Par unicité de la limite, on déduit que pour tout $\omega \in \Omega$, $L(\omega) = 1$. 
