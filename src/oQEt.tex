...
\texte{
Dans un camp militaire, deux unités médicales indépendantes sont disponibles pour soigner les soldats blessés. Deux soldats arrivent simultanément et se présentent respectivement aux deux unités médicales. On note X le temps d'attente du premier soldat et Y le temps d'attente du second soldat, en minutes. On supposera que X et Y suivent chacune une loi exponentielle de paramètre $\lambda=2$.
}

\begin{enumerate}
	\item \question{ Quel est le temps d'attente moyen pour chaque soldat ? }
	\reponse{ Puisque $X$ et $Y$ suivent chacune une loi exponentielle de paramètre $\lambda =2$, on a $\mathbb{E}(X)=\mathbb{E}(Y) = 0.5$ minutes, ce qui donne le temps d'attente moyen pour chaque soldat. }
	...
	\item \question{ Calculer la probabilité que la différence d'attente entre les deux soldats soit inférieure à 1 minute. }
