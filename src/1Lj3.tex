...
\texte{ On considère $X_1$, $X_2$ et $X_3$ trois variables aléatoires indépendantes de même loi uniforme sur l'intervalle $[0,2a]$, dont la densité est donc :
$$
f_{X_i}(t)=
\left\{\begin{array}{cl}
...
\begin{enumerate}
	...
	\indication{Utiliser les formules de la loi uniforme sur $[0,2a]$, puis la linéarité de l'espérance. Pour la variance de la moyenne, l'indépendance annule les covariances.}
	\reponse{ 
		Espérance de $X_i$:
	...
	\end{array}\right.
	$$ }
	\indication{Traduire $\{\max(X_1,X_2,X_3)\leq t\}$ comme l'intersection des trois événements $\{X_i\leq t\}$ ; l'indépendance permet de multiplier leurs probabilités. Dériver ensuite sur le support.}
	\reponse{ 
	...
	\item \question{ Calculer $\E(Z)$ et $\var(Z)$. Soit la variable aléatoire $T=\alpha Z$. Déterminer $\alpha$ de sorte que $\E(T)=\E(Y)$. }
	\indication{À partir de la densité trouvée, calculer $\E(Z)$ et $\E(Z^2)$. Puis utiliser $\E(\alpha Z)=\alpha\E(Z)$ pour fixer $\alpha$.}
	\reponse{ 
		Espérance de $Z$:
	...
	\indication{Utiliser la règle $\var(\alpha Z)=\alpha^2\var(Z)$ et les résultats précédents ; les deux estimateurs ont déjà la même espérance.}
	\reponse{ 
		On a
