...
\contenu{
	...
	\texte{
		En 1879, Michelson effectue 100 mesures de la vitesse de la lumière (en km$\cdot$s$^{-1}$). On modélise ces mesures comme des variables aléatoires $(X_i)$ i.i.d. gaussiennes d'espérance $\mu$ et d'écart-type $\sigma$ inconnus. Sur cet échantillon, on dispose des observations suivantes :
		\[
	...
	\begin{enumerate}
		...
		\item
		\question{On souhaite comparer les mesures de Michelson aux deux valeurs de référence de la vitesse de la lumière : $c_{1879} = 299\,840$ km$\cdot$s$^{-1}$ et $c_{2026} = 299\,792$ km$\cdot$s$^{-1}$. Effectuer deux tests statistiques distincts :
			\begin{itemize}
				\item un premier test avec $H_0 : \mu = c_{1879}$ contre $H_1 : \mu \neq c_{1879}$ ;
				\item un second test avec $H_0 : \mu = c_{2026}$ contre $H_1 : \mu \neq c_{2026}$.
			\end{itemize}
			Pour chaque test, construire la statistique, la région de rejet et conclure.}
		...
		\item
		\texte{On souhaite à présent tester l'écart-type $\sigma$ de la mesure. Sous $H_0 : \sigma = \sigma_0$, la statistique de test usuelle est :
			$$
			K = (n-1)\,\frac{S^2}{\sigma_0^2} \sim \chi^2(n-1).
			$$
			La table du $\chi^2$ n'étant pas disponible pour $d = n-1 = 99$ degrés de liberté, on admet l'approximation de Fisher : si $K \sim \chi^2(d)$, alors pour $d$ grand, la statistique :
			$$
			Z = \sqrt{2K} - \sqrt{2d-1}
			$$
