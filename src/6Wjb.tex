...
\contenu{
	\texte{ Deux urnes $U_1$ et $U_2$ contiennent des boules permettant des gains ou des pertes décrits comme suit : 
		\begin{itemize}
			\item dans l'urne $U_1$, il y a 1 boule \og perdant 1\euro{} \fg{}, 2 boules \og sans gain \fg{}, 3 boules \og gagnant 2\euro{} \fg{} ;
			\item dans l'urne $U_2$, il y a 3 boules \og perdant 1\euro{} \fg{}, 2 boules \og sans gain \fg{}, 1 boule \og gagnant 2\euro{} \fg{} ;
		\end{itemize}
	...
	\begin{enumerate}
		...
		\indication{Utiliser la formule du produit pour calculer toutes les valeurs $\prob(X=k,Y=k')$. }
		\reponse{ Pour calculer par exemple $\prob(X=1,Y=-1)$, on utilise la formule du produit :
