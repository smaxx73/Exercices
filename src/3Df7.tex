\uuid{3Df7}
\titre{Dimensionnement des chargements militaires : estimation et quantile}
\niveau{L2}
\module{Probabilités et Statistiques}
\chapitre{Probabilités continues}
\sousChapitre{Loi normale}
\theme{Loi normale, estimation de paramètres, quantiles}
...
\contenu{
	...
	\texte{On modélise par une variable aléatoire \( X \) la consommation de chargeurs par mission d'un fantassin. On suppose que \( X \) suit une loi normale continue \( X \sim \mathcal{N}(\mu, \sigma^2) \). Des journaux d'entraînement indiquent qu'environ 20\% des missions consomment au plus 2 chargeurs, et qu'environ 30\% des missions en consomment (strictement) plus de 4.}
	...
	\begin{enumerate}
		...
		\indication{Utiliser le quantile d'ordre \(0,95\) de la loi normale centrée réduite.}
		\reponse{
			Pour couvrir 95\% des situations, on cherche \( m \) entier tel que \( P(X \leq m) \geq 0,95 \). Comme \(X\) est modélisée par une loi normale continue,
			\[
			P(X \leq m) = \Phi\left(\frac{m - \mu}{\sigma}\right) \geq 0,95 \iff \frac{m - \mu}{\sigma} \geq z_{0,95} \approx 1,645.
			\]
			Ainsi, \( m \geq \mu + 1,645 \sigma \approx 3,23 + 1,645 \times 1,46 \approx 5,64 \). Le plus petit entier est donc \( m = 6 \).
		}
