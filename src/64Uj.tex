...
\begin{enumerate}
	...
	\item \question{Justifier l'existence et l'unicité globale d'une solution $y$ de classe $\mathcal{C}^1$. 
...
%	où $f^{[0]} = f$ et $f^{[k+1]} = \frac{\partial f^{[k]}}{\partial t} + f \frac{ \partial f^{[k]}}{\partial y}$.
    ...
    \item \question{Proposer un algorithme qui donne une approximation de $y(1)$ pour $h = 10^{-6}$ en utilisant le schéma numérique ci-dessus. 
\end{enumerate}}
}