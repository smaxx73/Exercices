...
\begin{enumerate}
	...
	\reponse{Soit la variable aléatoire $X$ donnant la somme des poids des 100 passagers. Si on note $Y_i$ le poids du $i$-ème passager, on a $X = \sum_{i=1}^{100} Y_i$. Par conséquent, $\mathbb{E}(X) = 7100$ et $V(X) = 100 \times 7^2 = 4900$. 
...
\begin{align*}
  \PP\left(X > 7300\right) &= \PP\left(\frac{X-7000}{70} > \frac{7300-7000}{70}\right) \\
  &\approx  \PP\left(Z > \frac{300}{70} \approx 4{,}2\right) \\
  &\approx 0
\end{align*}
Il n'y a quasiment aucune chance que le poids total des passagers dépasse $7{}3$ tonnes.	
	...
	\reponse{On cherche à estimer une fréquence à partir d'un échantillon de taille $40$. La fréquence observée dans l'échantillon est $f_{obs} = \frac{23}{40}$. On peut donc utiliser la formule du cours : 
		...
		en remplaçant $u_{\alpha/2}$ par $1{,}96$ pour une confiance de $95\%$, on obtient numériqument $I_{conf} \approx [0.42 ; 0.73]$.} 
\end{enumerate}