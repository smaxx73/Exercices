...
\contenu{
	...
	\texte{
		...
		\[
		f_\theta(x) = \frac{x}{\theta^2} e^{-x^2/(2\theta^2)} \mathbf{1}_{[0,+\infty[}(x).
		\]
		Soit $X_1, \ldots, X_n$ un échantillon i.i.d. de loi $f_\theta$. On pose
		\[
	...
	\begin{enumerate}
		...
		\item
		\question{Calculer l'écart quadratique moyen de $\hat{\theta}^2_n$ et montrer que $\hat{\theta}^2_n$ converge en moyenne quadratique vers $\theta^2$.}
		\reponse{
		...
		\item
		\question{En déduire un intervalle de confiance de niveau \textbf{au moins} $95\%$ pour $\theta^2$ via l'inégalité de Bienaymé-Tchebychev. Comparer sa largeur à celle des intervalles obtenus aux questions 8 et 10 : quel est le prix à payer pour ne pas utiliser la loi exacte de $\hat{\theta}^2_n$ ?}
		\reponse{
