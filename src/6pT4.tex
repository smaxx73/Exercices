...
\indication{Calculer séparément $\E(X^2)=\sum_i x_i^2\prob(X=x_i)$, puis employer $V(X)=\E(X^2)-\E(X)^2$.}
\reponse{
On calcule d’abord
\[
...
\end{enumerate}
\texte{On utilise désormais les résultats de la variable X pour un test de contrôle qualité sur une chaine de
...
\indication{Les événements « jeton pair » et « jeton impair » forment une partition : décomposer $D$ suivant ces deux cas.}
\reponse{
...
\indication{On cherche une probabilité conditionnelle $P(A\mid D)$. Utiliser $P(A\mid D)=\frac{P(A)P(D\mid A)}{P(D)}$, la question précédente fournissant le dénominateur.}
\reponse{
On cherche
\[
P(A\mid D).
