...
$$T_1 = \frac{1}{5}\sum_{i=1}^{5} X_i \qquad T_2 = \frac{1}{3}(X_1 + X_2) + \frac{1}{6}(X_3 + X_4 + X_5)$$
...
\reponse{
	...
	Donc $V \sim \mathcal{N}(0,2)$
}
\item \question{ Calculer $\prob(-4{,}5 \leq V \leq 4{,}5)$ avec 8 chiffres significatifs.}
\reponse{
	\begin{align*}
		\prob(-4{,}5 \leq V \leq 4{,}5) &= \prob\left(-\frac{4{,}5}{2} \leq \frac{V}{2} \leq \frac{4{,}5}{2}\right) \\
		&= \prob(-2{,}25 \leq \frac{V}{2} \leq 2{,}25) \\
		&= 2\Phi(2{,}25) - 1 \\
		&\approx 2 \times 0{,}98758067 - 1 \\
		&\approx 0{,}97516134
	\end{align*}
}
...
\reponse{
	...
	Pour $T_2$ :
	\begin{align*}
		\E(T_2) &= \frac{1}{3}(\E(X_1) + \E(X_2)) + \frac{1}{6}(\E(X_3) + \E(X_4) + \E(X_5)) \\
		&= \frac{2\mu}{3} + \frac{3\mu}{6} = \mu
	\end{align*}
	Donc $T_2$ est aussi sans biais.
}

\item \question{Calculer la variance de $T_1$ et $T_2$. En déduire l'estimateur le plus efficace.}
\reponse{
	...
	Pour $T_2$ :
	\begin{align*}
		\V(T_2) &= \frac{1}{9}\V(X_1 + X_2) + \frac{1}{36}\V(X_3 + X_4 + X_5) \\
		&= \frac{1}{9}(8) + \frac{1}{36}(12) = \frac{8}{9} + \frac{12}{36} = \frac{32}{36} \approx 0.89
	\end{align*}
	
	Comme $\frac{4}{5} < \frac{32}{36}$, $T_1$ est plus efficace que $T_2$.
}
...
\reponse{
	...
	D'après le cours, $\frac{U}{\sigma^2}$ suit une loi du $\chi^2$ à $n-1 = 4$ degrés de liberté.
